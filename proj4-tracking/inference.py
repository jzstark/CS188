...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # grab all factors where we know the evidence variables (to reduce the size of the tables)
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        #print(evidenceVariablesSet, queryVariablesSet, eliminationVariables)
        #for c in currentFactorsList:
        #    print(c)

        #for elimVar in eliminationVariables:
        for elimVar in eliminationOrder:
            # join all factors mentioning the hidden variable elimVar
            currentFactorsList, jointFactor = joinFactorsByVariable(currentFactorsList, elimVar)
            # eliminate elimVar
            if len(jointFactor.unconditionedVariables()) > 1:
                elimJointFactor = eliminate(jointFactor, elimVar)
                currentFactorsList.append(elimJointFactor)
...
class DiscreteDistribution(dict):
    ...
    def sample(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        (k, v) = tuple(zip(*self.items()))
        return random.choices(list(k), weights=list(v), k=1)[0]
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    1. the time elapse update which corresponds to determining B'(Wi+1) from B(Wi)
    2. the observation update which corresponds to determining B(Wi+1) from B'(Wi+1)
    """
    def initializeUniformly(self, gameState):
        """
...
class ParticleFilter(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        newParticles = [(0,0)] * self.numParticles
